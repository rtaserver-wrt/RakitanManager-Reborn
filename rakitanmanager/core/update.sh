...
backup_system() {
    ...
    if [ -d "/usr/share/rakitanmanager" ]; then
        cp -r "/usr/share/rakitanmanager" "$BACKUP_DIR/" && \
        log_message "Backed up /usr/share/rakitanmanager"
    fi
    
    if [ -d "/www/rakitanmanager" ]; then
        cp -r "/www/rakitanmanager" "$BACKUP_DIR/" && \
        log_message "Backed up /www/rakitanmanager"
    fi
    
    # Backup config
    if [ -f "$CONFIG_FILE" ]; then
        cp "$CONFIG_FILE" "$BACKUP_DIR/" && \
        log_message "Backed up config file"
    fi
}
...
install_files() {
    ...
    if [ -d "$TEMP_DIR/rakitanmanager" ]; then
        ...
        cp -rf "$TEMP_DIR/." /usr/share/rakitanmanager/ && \
        log_message "Installed files (alternative structure)"
    fi
...
main() {
    ...
    # Step 3: Backup (33%)
    update_progress 3 "Creating backup" 33
    backup_system
    
    # Step 4: Download (44%)
    update_progress 4 "Downloading repository" 44
    if ! download_repository; then
        update_progress 4 "Download failed" 44 "failed"
        exit 1
    fi
    
    # Step 5: Install files (55%)
    update_progress 5 "Installing files" 55
    install_files
    
    # Step 6: LuCI integration (66%)
    update_progress 6 "Creating LuCI integration" 66
    create_luci_integration
    
    # Step 7: Validate (77%)
    update_progress 7 "Validating installation" 77
    validate_installation
    
    # Step 8: Start service (88%)
    update_progress 8 "Starting service" 88
    start_service
    
    # Step 9: Cleanup (100%)
    update_progress 9 "Cleaning up" 100
    cleanup
